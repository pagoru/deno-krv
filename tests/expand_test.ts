...
Deno.test("expand: forward, a reference field becomes the row it points to", async () => {
  ...
  const noEditor = (await db.get(draft.key, { expand: { editor: "editorId" } }))
    .value!;
  assertEquals(noEditor.editor, null);
  db.close();
});

Deno.test("expand: reverse, rows of another table pointing here become a list", async () => {
  ...
  assertEquals(author.drafts.map((d) => d.text), ["notes"]);
  db.close();
});

Deno.test("expand: a table keyed by the reference (1:1) gives one row or null", async () => {
...
Deno.test("expand: options per reverse expand, and nesting", async () => {
  ...
  assertEquals(le.books.map((b) => b.title), ["The Dispossessed"]);
...
Deno.test("expand: with list, find and values: false", async () => {
  ...
  assertEquals(books.map((b) => b.author.name), [
    "Le Guin",
    "Le Guin",
    "Herbert",
  ]);
  ...
  let streamed = 0;
  for await (
    const book of db.list(["books"], { expand: { reviews: "reviews.bookId" } })
  ) streamed += book.reviews.length;
  assertEquals(streamed, 2);
  db.close();
});

Deno.test("expand: invalid expands fail at the call, and at compile time", async () => {
  const db = await open();
