...
Deno.test("migrations: typed API, raw access and transforms inside up", async () => {
  const path = await tempPath();
  const tables = [
    table({
...
Deno.test("migrations: events, in order, with the backup in place", async () => {
  const path = await tempPath();
  const events: string[] = [];
  const db = await openKRV({
    path,
...
Deno.test("schema changes: stale rows without a migration refuse to open", async () => {
  const path = await tempPath();
  ...
  const v2 = await openKRV({
    ...
    migrations: [
      migration("add-age", async (db) => {
        for await (const { key, value } of db.raw.list({ prefix: ["users"] })) {
...
Deno.test("schema changes: a migration leaving invalid rows is rolled back", async () => {
  const path = await tempPath();
...
Deno.test("schema changes: a new index is built for existing rows", async () => {
  const path = await tempPath();
  const v1 = await openKRV({ path, tables: usersV1 });
...
Deno.test("schema changes: a unique index over duplicate values refuses to open", async () => {
  const path = await tempPath();
  const v1 = await openKRV({ path, tables: usersV1 });
...
Deno.test("recovery: a backup left by a crash is restored at open", async () => {
  const path = await tempPath();
  const db = await openKRV({ path, tables: usersV1 });
  ...
  assertEquals((await restored.list(["users"])).map((u) => u.id), ["a"]);
  assert(!(await exists(`${path}.backup`)));
  restored.close();
});

Deno.test("lock: concurrent opens wait, and a migration runs once", async () => {
  const path = await tempPath();
  let runs = 0;
  const migrations = [
...
Deno.test("lock: an abandoned lock is taken over after lockTimeout", async () => {
  const path = await tempPath();
