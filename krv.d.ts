...
export type KrvKeyPart = Uint8Array | string | number | bigint | boolean | symbol;
 ...
 */
export type KrvListResult<T, K = KrvKey> = AsyncIterable<KrvEntry<T, K>> & PromiseLike<KrvEntry<T, K>[]>;
...
export type KrvListOptions<Row, Where = {
...
export type KrvPatch<T> = {
	[K in keyof T]?: T[K] extends Replaced ? T[K] | undefined : T[K] extends object ? string extends keyof T[K] ? T[K] | undefined : KrvPatch<T[K]> | undefined : T[K] | undefined;
};
/** Runtime shape of an `expand` entry (typed per table by `KrvExpand`). */
export type KrvExpandSpec = string | {
	from: string;
 ...
 */
export type KrvSpec = string | number | boolean | null | undefined | readonly KrvSpec[] | {
...
type Trim<S> = S extends ` ${infer R}` ? Trim<R> : S extends `${infer R} ` ? Trim<R> : S;
...
type BaseOf<V> = V extends readonly [
	infer Base,
	unknown
] ? Base : never;
...
type StringSpecType<S extends string, F> = S extends KrvBuiltin ? Builtins[S] : S extends `|${infer L}|` ? L : S extends `{${string}}` ? string : S extends `${infer N}(${string})` ? ValidatorType<FindValidator<N, F>, F> : ValidatorType<FindValidator<S, F>, F>;
type ValidatorType<K, F> = [
	K
] extends [
	never
] ? never : K extends keyof F ? KrvSpecType<BaseOf<F[K]>, KrvNoValidators> : never;
/** TypeScript type of a value spec. `F` holds the custom validators. */
export type KrvSpecType<S, F> = S extends string ? StringSpecType<S, F> : S extends readonly (infer E)[] ? KrvSpecType<E, F> : S extends number | boolean | null | undefined ? S : S extends object ? KrvObjectType<S, F> : never;
/** `"tags[]?"` → `"tags"` */
export type KrvFieldName<K> = K extends `${infer R}?` ? KrvFieldName<R> : K extends `${infer R}[]` ? KrvFieldName<R> : K extends `${infer R}{}` ? KrvFieldName<R> : K;
type Wrap<K, T> = K extends `${infer R}?` ? Wrap<R, T> : K extends `${infer R}[]` ? Wrap<R, T>[] : K extends `${infer R}{}` ? Record<string, Wrap<R, T>> : T;
type IsOptional<K, S, F> = K extends `${string}?` ? true : undefined extends KrvSpecType<S, F> ? true : false;
type IsGenerated<K, S> = K extends KrvFieldName<K> ? S extends "id" ? true : S extends `{${string}.${string}}` ? false : S extends `{${string}}` ? true : false : false;
type Field<K, S, F> = Wrap<K, KrvSpecType<S, F>>;
export type KrvObjectType<S, F> = Prettify<{
	-readonly [K in keyof S as IsOptional<K, S[K], F> extends true ? never : KrvFieldName<K>]-?: Field<K, S[K], F>;
} & {
	-readonly [K in keyof S as IsOptional<K, S[K], F> extends true ? KrvFieldName<K> : never]?: Field<K, S[K], F>;
}>;
/** What `insert`/`set` accept: generated fields may be left out. */
export type KrvInputType<S, F> = Prettify<{
	-readonly [K in keyof S as IsOptional<K, S[K], F> extends true ? never : IsGenerated<K, S[K]> extends true ? never : KrvFieldName<K>]-?: Field<K, S[K], F>;
} & {
	-readonly [K in keyof S as IsOptional<K, S[K], F> extends true ? KrvFieldName<K> : IsGenerated<K, S[K]> extends true ? KrvFieldName<K> : never]?: Field<K, S[K], F>;
}>;
type ArgTemplate<T> = [
	T
] extends [
	never
] ? AnyArgTemplate : unknown extends T ? AnyArgTemplate : (T extends number ? `${number}` : never) | (T extends string ? `|${string}|` : never) | (T extends boolean ? `${T}` : never) | (T extends null ? "null" : never);
type AnyArgTemplate = `${number}` | `|${string}|` | "true" | "false" | "null";
type JoinArgs<Params, Args> = Params extends [
	infer P,
	...infer Rest
] ? Rest extends [
] ? ArgTemplate<P extends keyof Args ? Args[P] : never> : `${ArgTemplate<P extends keyof Args ? Args[P] : never>}, ${JoinArgs<Rest, Args>}` : "";
type ValidatorUsage<F> = {
	[K in keyof F & string]: DeclParams<K> extends [
	] ? K : `${DeclName<K>}(${JoinArgs<DeclParams<K>, ArgsOf<F[K]>>})`;
}[keyof F & string];
type ValidString<F> = KrvBuiltin | `|${string}|` | `{${string}}` | ValidatorUsage<F>;
/** A value spec where every type name exists and every call is well-formed. */
export type KrvSpecFor<F> = ValidString<F> | number | boolean | null | undefined | readonly KrvSpecFor<F>[] | {
...
type StripTransform<K, T> = [
	CharOf<K, T>
] extends [
	never
] ? StripOptional<K> : StripOptional<K> extends `${infer R}${CharOf<K, T>}` ? R : StripOptional<K>;
/** Name of a table's top-level field: `"tokens[]&?"` → `"tokens"`. */
export type KrvTopFieldName<K, T> = KrvFieldName<StripTransform<K, T>>;
type ItemOutput<K, S, V, T> = [
	CharOf<K, T>
] extends [
	never
] ? KrvSpecType<S, V> : T[CharOf<K, T> & keyof T] extends {
	load: (stored: never) => unknown;
} ? KrvSpecType<S, V> : string;
...
export type KrvTableRow<S, V, T, TS> = Prettify<{
	-readonly [K in keyof S as IsOptional<K, S[K], V> extends true ? never : KrvTopFieldName<K, T>]-?: TopField<K, S[K], V, T>;
} & {
	-readonly [K in keyof S as IsOptional<K, S[K], V> extends true ? KrvTopFieldName<K, T> : never]?: TopField<K, S[K], V, T>;
} & Timestamps<TS>>;
...
export type KrvTableInput<S, V, T, TS> = Prettify<{
	-readonly [K in keyof S as IsOptional<K, S[K], V> extends true ? never : IsGenerated<K, S[K]> extends true ? never : KrvTopFieldName<K, T>]-?: TopFieldInput<K, S[K], V, T>;
} & {
	-readonly [K in keyof S as IsOptional<K, S[K], V> extends true ? KrvTopFieldName<K, T> : IsGenerated<K, S[K]> extends true ? KrvTopFieldName<K, T> : never]?: TopFieldInput<K, S[K], V, T>;
} & Partial<Timestamps<TS>>>;
type WhereValue<X> = X extends Date | Uint8Array | readonly unknown[] ? X : X extends object ? {
	[K in keyof X]?: WhereValue<X[K]>;
} : X;
type Searchable<K, T, Using> = [
	CharOf<K, T>
] extends [
	never
] ? true : T[CharOf<K, T> & keyof T] extends {
	deterministic: true;
} ? true : KrvTopFieldName<K, T> extends Using ? true : false;
...
export type KrvTableWhere<S, V, T, TS, Using = never> = Prettify<{
	-readonly [K in keyof S as Searchable<K, T, Using> extends true ? KrvTopFieldName<K, T> : never]?: WhereValue<TopFieldInput<K, S[K], V, T>>;
} & Partial<Timestamps<TS>>>;
...
export type KrvTable<Key extends readonly string[] = readonly string[], Schema extends KrvSchemaDef = KrvSchemaDef, Timestamps extends boolean = boolean, Indexes extends KrvIndexes = KrvIndexes> = {
...
} ? boolean extends TS ? true : TS extends false ? false : true : true;
...
export type KrvTableValue<Table, E = KrvEnv> = Table extends AnyTable<readonly string[], infer Schema> ? KrvTableRow<Schema, EnvV<E>, EnvT<E>, TimestampsOf<Table>> : never;
/** What `insert`/`set` accept for a table. */
export type KrvTableInputValue<Table, E = KrvEnv> = Table extends AnyTable<readonly string[], infer Schema> ? KrvTableInput<Schema, EnvV<E>, EnvT<E>, TimestampsOf<Table>> : never;
...
/** `where` of a table. */
export type KrvTableWhereValue<Table, E = KrvEnv> = Table extends AnyTable<readonly string[], infer Schema> ? KrvTableWhere<Schema, EnvV<E>, EnvT<E>, TimestampsOf<Table>, UsingFields<Table> & string> : never;
...
type Literals<Key> = Key extends readonly [
	infer Head,
	...infer Rest
] ? Head extends `{${string}}` ? Literals<Rest> : [
	Head,
	...Literals<Rest>
] : [
];
/** Literal parts of a table's key, used by `insert` and `list`: `["posts"]`. */
export type KrvTableLiterals<Table> = Table extends AnyTable<infer Key> ? Readonly<Literals<Key>> : never;
export type KrvAnyKey<Tables extends KrvTables> = KrvRowKey<Tables[number]>;
export type KrvAnyLiterals<Tables extends KrvTables> = KrvTableLiterals<Tables[number]>;
type TableAtKeyOf<Table, Key> = Table extends unknown ? Key extends KrvRowKey<Table> ? Table : never : never;
export type KrvTableAtKey<Tables extends KrvTables, Key> = TableAtKeyOf<Tables[number], Key>;
type TableAtLiteralsOf<Table, Literals> = Table extends unknown ? [
	Literals
] extends [
	KrvTableLiterals<Table>
] ? [
	KrvTableLiterals<Table>
] extends [
	Literals
] ? Table : never : never : never;
export type KrvTableAtLiterals<Tables extends KrvTables, Literals> = TableAtLiteralsOf<Tables[number], Literals>;
export type KrvValueAt<Tables extends KrvTables, E, Key> = KrvTableValue<KrvTableAtKey<Tables, Key>, E>;
export type KrvInputAt<Tables extends KrvTables, E, Key> = KrvTableInputValue<KrvTableAtKey<Tables, Key>, E>;
export type KrvValueAtLiterals<Tables extends KrvTables, E, Literals> = KrvTableValue<KrvTableAtLiterals<Tables, Literals>, E>;
export type KrvInputAtLiterals<Tables extends KrvTables, E, Literals> = KrvTableInputValue<KrvTableAtLiterals<Tables, Literals>, E>;
export type KrvWhereAtLiterals<Tables extends KrvTables, E, Literals> = KrvTableWhereValue<KrvTableAtLiterals<Tables, Literals>, E>;
export type KrvRowKeyAtLiterals<Tables extends KrvTables, Literals> = KrvRowKey<KrvTableAtLiterals<Tables, Literals>>;
/** Top-level field names of the table at a key, for `db.compare`. */
export type KrvFieldAt<Tables extends KrvTables, E, Key> = keyof KrvValueAt<Tables, E, Key> & string;
/**
...
export type KrvUntypedDb = {
	...
	set(key: KrvKey, value: any, options?: KrvSetOptions): Promise<KrvCommitResult>;
	insert(literals: KrvKey, value: any, options?: KrvInsertOptions): Promise<KrvCommitResult & {
		key: KrvKey;
		value: any;
	}>;
	update(key: KrvKey, patch: any, options?: KrvUpdateOptions): Promise<any>;
	delete(key: KrvKey, options?: KrvDeleteOptions): Promise<void>;
	list(literals: KrvKey, options?: KrvListOptions<any, any>): KrvListResult<any>;
	find(literals: KrvKey, options?: Omit<KrvListOptions<any, any>, "limit">): Promise<any | null>;
 ...
 */
export type KrvMigrationDb<Db = KrvUntypedDb> = Omit<Db, "raw" | "transforms" | "close"> & {
	/**
...
export type KrvMigrationSource = AnyMigration | {
	default: AnyMigration;
} | Promise<AnyMigration | {
	default: AnyMigration;
}>;
...
export type KrvEvents = {
	...
	afterMigrations?: (event: {
		applied: KrvLoadedMigration[];
	}) => unknown;
};
...
type JoinDot<T> = T extends [
	infer H extends string
] ? H : T extends [
	infer H extends string,
	...infer R
] ? `${H}.${JoinDot<R>}` : "";
type InitOf<T> = T extends [
	...infer I,
	unknown
] ? I : [
];
/** A table's name: its key's literal parts, joined by dots. */
export type KrvTableName<Table> = KrvTableLiterals<Table> extends readonly [
	...infer L
] ? JoinDot<L> : never;
type TableByName<Tables extends KrvTables, Name> = TableByNameOf<Tables[number], Name>;
type TableByNameOf<Table, Name> = Table extends unknown ? KrvTableName<Table> extends Name ? Table : never : never;
...
type RefString<V> = V extends `{${infer R}}` ? R extends `${string}.${string}` ? R : never : V extends readonly (infer U)[] ? RefString<U> : never;
type RefFields<Table> = {
	[K in keyof SchemaOf<Table> as [
		RefString<SchemaOf<Table>[K]>
	] extends [
		never
	] ? never : KrvFieldName<K>]: RefString<SchemaOf<Table>[K]>;
};
type TargetName<Ref> = Ref extends string ? JoinDot<InitOf<SplitDot<Ref>>> : never;
type Placeholders<Key> = Key extends readonly (infer P)[] ? P extends `{${infer Name}}` ? Name : never : never;
type ReverseRefs<Tables extends KrvTables, Table> = ReverseRefsOf<Tables[number], Table>;
type ReverseRefsOf<Source, Table> = Source extends unknown ? {
	[F in keyof RefFields<Source>]: TargetName<RefFields<Source>[F]> extends KrvTableName<Table> ? `${KrvTableName<Source>}.${F & string}` : never;
}[keyof RefFields<Source>] : never;
type ForwardRefs<Table> = keyof RefFields<Table> & string;
type Depth = unknown[];
type Next<D extends Depth> = [
	...D,
	unknown
];
type ForwardEntry<Tables extends KrvTables, E, Table, F, D extends Depth> = F extends string ? {
	/** A reference field of this row: expands to the row it points to. */
	from: F;
	expand?: KrvExpand<Tables, E, TableByName<Tables, TargetName<RefFields<Table>[F & keyof RefFields<Table>]>>, Next<D>>;
} : never;
type ReverseEntry<Tables extends KrvTables, E, R, D extends Depth> = R extends string ? ReverseEntryOf<Tables, E, R, TableByName<Tables, JoinDot<InitOf<SplitDot<R>>>>, D> : never;
...
type ExpandEntry<Tables extends KrvTables, E, Table, D extends Depth> = ForwardRefs<Table> | ReverseRefs<Tables, Table> | ForwardEntry<Tables, E, Table, ForwardRefs<Table>, D> | ReverseEntry<Tables, E, ReverseRefs<Tables, Table>, D>;
/**
 ...
 */
export type KrvExpand<Tables extends KrvTables, E, Table, D extends Depth = [
]> = D["length"] extends 4 ? never : Record<string, ExpandEntry<Tables, E, Table, D>>;
type FromOf<X> = X extends string ? X : X extends {
	from: infer F;
} ? F : never;
type NestedOf<X> = X extends {
	expand: infer N;
} ? N : Record<never, never>;
type ForwardResult<Tables extends KrvTables, E, Table, X> = TableByName<Tables, TargetName<RefFields<Table>[FromOf<X> & keyof RefFields<Table>]>> extends infer Target ? KrvExpanded<Tables, E, Target, KrvTableValue<Target, E>, NestedOf<X>> | (null extends KrvTableValue<Table, E>[FromOf<X> & keyof KrvTableValue<Table, E>] ? null : undefined extends KrvTableValue<Table, E>[FromOf<X> & keyof KrvTableValue<Table, E>] ? null : never) : never;
type ReverseResult<Tables extends KrvTables, E, Table, X> = TableByName<Tables, JoinDot<InitOf<SplitDot<FromOf<X> & string>>>> extends infer Source ? KrvExpanded<Tables, E, Source, KrvTableValue<Source, E>, NestedOf<X>> extends infer Row ? [
	Placeholders<KeyOf<Source>>
] extends [
	Placeholders<KeyOf<Table>>
] ? Row | null : Row[] : never : never;
type ExpandResult<Tables extends KrvTables, E, Table, X> = FromOf<X> extends `${string}.${string}` ? ReverseResult<Tables, E, Table, X> : ForwardResult<Tables, E, Table, X>;
type Prettify$1<T> = {
	[K in keyof T]: T[K];
} & {};
...
export interface KrvDatabase<in out Tables extends KrvTables, in out E> {
	 ...
	 * ```
	 */
	get: <const Key extends KrvAnyKey<Tables>, const X extends Record<string, unknown> = Record<never, never>>(key: Key, options?: KrvGetOptions & {
		expand?: X & KrvExpand<Tables, E, KrvTableAtKey<Tables, Key>> & NoInfer<KrvExpandNoClash<X, KrvTableAtKey<Tables, Key>, E>>;
	}) => Promise<KrvEntryMaybe<KrvExpanded<Tables, E, KrvTableAtKey<Tables, Key>, KrvValueAt<Tables, E, Key>, X>, Key>>;
	/**
	 ...
	 * ```
	 */
	set: <const Key extends KrvAnyKey<Tables>>(key: Key, value: KrvInputAt<Tables, E, Key>, options?: KrvSetOptions<KrvFieldAt<Tables, E, Key>>) => Promise<KrvCommitResult>;
	/**
	 ...
	 * ```
	 */
	update: <const Key extends KrvAnyKey<Tables>>(key: Key, patch: KrvPatch<KrvInputAt<Tables, E, Key>> | ((row: KrvValueAt<Tables, E, Key>) => KrvPatch<KrvInputAt<Tables, E, Key>>), options?: KrvUpdateOptions<KrvFieldAt<Tables, E, Key>>) => Promise<KrvValueAt<Tables, E, Key>>;
	/**
	 ...
	 * ```
	 */
	insert: <const Literals extends KrvAnyLiterals<Tables>>(literals: Literals, value: KrvInputAtLiterals<Tables, E, Literals>, options?: KrvInsertOptions<keyof KrvValueAtLiterals<Tables, E, Literals> & string>) => Promise<KrvCommitResult & {
	 ...
	 * ```
	 */
	delete: <const Key extends KrvAnyKey<Tables>>(key: Key, options?: KrvDeleteOptions) => Promise<void>;
	/**
	 ...
	 * ```
	 */
	list: <const Literals extends KrvAnyLiterals<Tables>, const Values extends boolean = true, const X extends Record<string, unknown> = Record<never, never>>(literals: Literals, options?: KrvListOptions<KrvValueAtLiterals<Tables, E, Literals>, KrvWhereAtLiterals<Tables, E, Literals>> & {
		values?: Values;
		expand?: X & KrvExpand<Tables, E, KrvTableAtLiterals<Tables, Literals>> & NoInfer<KrvExpandNoClash<X, KrvTableAtLiterals<Tables, Literals>, E>>;
	}) => Values extends false ? KrvListResult<KrvExpanded<Tables, E, KrvTableAtLiterals<Tables, Literals>, KrvValueAtLiterals<Tables, E, Literals>, X>, KrvRowKeyAtLiterals<Tables, Literals>> : KrvValuesResult<KrvExpanded<Tables, E, KrvTableAtLiterals<Tables, Literals>, KrvValueAtLiterals<Tables, E, Literals>, X>>;
	/**
	 ...
	 * ```
	 */
	find: <const Literals extends KrvAnyLiterals<Tables>, const Values extends boolean = true, const X extends Record<string, unknown> = Record<never, never>>(literals: Literals, options?: Omit<KrvListOptions<KrvValueAtLiterals<Tables, E, Literals>, KrvWhereAtLiterals<Tables, E, Literals>>, "limit"> & {
		values?: Values;
		expand?: X & KrvExpand<Tables, E, KrvTableAtLiterals<Tables, Literals>> & NoInfer<KrvExpandNoClash<X, KrvTableAtLiterals<Tables, Literals>, E>>;
	}) => Promise<(Values extends false ? KrvEntry<KrvExpanded<Tables, E, KrvTableAtLiterals<Tables, Literals>, KrvValueAtLiterals<Tables, E, Literals>, X>, KrvRowKeyAtLiterals<Tables, Literals>> : KrvExpanded<Tables, E, KrvTableAtLiterals<Tables, Literals>, KrvValueAtLiterals<Tables, E, Literals>, X>) | null>;
	/**
	 ...
	 * ```
	 */
	compare: <const Key extends KrvAnyKey<Tables>>(key: Key, fieldName: KrvFieldAt<Tables, E, Key>, plain: unknown) => Promise<boolean>;
	/** Closes the database. */
	close: () => void;
}
type TablesOf<Db> = Db extends KrvDatabase<infer Tables, infer _E> ? Tables : never;
 ...
 * ```
 */
export type KrvRow<Db, Literals extends KrvAnyLiterals<TablesOf<Db>>> = Db extends KrvDatabase<infer Tables, infer E> ? KrvValueAtLiterals<Tables, E, ReadonlyLiterals<Literals>> : never;
/**
 ...
 * ```
 */
export type KrvInput<Db, Literals extends KrvAnyLiterals<TablesOf<Db>>> = Db extends KrvDatabase<infer Tables, infer E> ? KrvInputAtLiterals<Tables, E, ReadonlyLiterals<Literals>> : never;
/**
...
export type KrvWithDefaultTransforms<T> = Omit<KrvDefaultTransforms, keyof T> & T;
type ValidatorMap<V, F> = {
	[K in keyof V]: readonly [
		V[K],
		K extends keyof F ? F[K] extends readonly [
			unknown,
			infer Fn
		] ? Fn : never : never
	];
...
export type KrvConfig<V, F, T, Tables extends readonly unknown[]> = {
	 ...
	 */
	tables?: {
		[I in keyof Tables]: Tables[I] & KrvTable<readonly string[], KrvSchemaFor<ValidatorMap<V, F>>>;
	};
};
/**
 ...
 * ```
 */
export declare const defineKRV: <const V extends Record<string, KrvSpec> = KrvNoValidators, F extends {
	[K in keyof V]: readonly [
		unknown,
		unknown
	];
...
}, const T extends KrvTransforms = KrvNoTransforms, const Tables extends readonly KrvTable[] = readonly [
]>(config: KrvConfig<V, F, T, Tables>) => KrvDefinedConfig<F, T, Tables> & KrvConfigTypes<Tables, Env<V, F, T>>;
/**
 * Opens a database.
 *
 ...
 * ```
 */
export declare const openKRV: <const V extends Record<string, KrvSpec> = KrvNoValidators, F extends {
	[K in keyof V]: readonly [
		unknown,
		unknown
	];
 ...
 * ```
 */
export declare const table: <const Key extends readonly string[], const Schema extends KrvSchemaDef, const Timestamps extends boolean = true, const Indexes extends Record<string, KrvIndex> = Record<never, never>>(definition: {
	key: Key;
	schema: Schema;
	/** Automatic `createdAt` / `updatedAt` fields. Default `true`. */
...
export declare const parseMigrationName: (fileName: string) => ParsedMigrationName | null;
/**
 ...
 * ```
 */
export declare const nextMigrationName: (existing: Iterable<string>, name?: string, now?: Date, number?: number) => string;
